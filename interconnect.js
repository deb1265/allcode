...
const nycsformsfileId = '1ZuQMHD0ekITNYgcRwGAls8yW1nN8gvq3';
...
async function generatePvWattsReport(address, systemCapacity, losses, tilt, azimuth, index) {
    ...
    function generatePDF(values, filename) {
        ...
        const pvWattsLogoPath = '/tmp/pvwatts/pvwatts logo.JPG'; // Path to the PV Watts logo
        const nrelLogoPath = './nrel logo.png'; // Path to the NREL logo
    ...
    async function main() {
        ...
        const filename = `PVWatts_Array${index + 1}.pdf`;
...
async function uploadFile(client, filePath, folderId,NYC_LI,Totalstring,Name,METER) {
    try {
        const drive = google.drive({ version: 'v3', auth: client });
        const ext = path.extname(filePath);
         let signedFormsFolderId = await createFolder(`${Name}-interconnect-${METER}`, folderId, drive); 
        ...
        if (ext === '.pdf') {
            ...
            if (NYC_LI !== 'NYC') {
                parts = {
                    'Pseg_AppendixA': [0, 9],
                    'Pseg_AppendixH': [10, 10],
                    'Pseg_Letter_of_auth': [11, 11],
                    'Pseg_sitecontrol': [16, 16],
                    'Pseg_invspecs': [38, 39]
                };
				if (Totalstring == 1) {
                    parts['Pseg_ELD'] = [42, 42];
                } else if (Totalstring == 2) {
                    parts['Pseg_ELD'] = [43, 43];
                } else if (Totalstring == 3) {
                    parts['Pseg_ELD'] = [44, 44];
                } else if (Totalstring == 4) {
                    parts['Pseg_ELD'] = [40, 40];
                }
            } else {
                parts = {
                    'Coned_AppendixA': [19, 25],
                    'Coned_AppendixG': [26, 35],
                    'Coned_letterofauth': [36, 36],
                    'Coned_verificationtesting': [37, 37],
                    'Coned_ELD': [40, 40],
                    'Coned_invspecs': [38, 39]
                };
            }
...
async function caspiomain(CIDvalue, accessToken) {
    try {
        ...
        return { Firstname, Lastname, FullName, Address, House, Street, City, Zip, Panel, NYC_LI, Size, contractfileid, Contract_Price, Number_Of_Panels, InterconnectionMeter1, InterconnectionMeter2, InterconnectionMeter3, InterconnectionAcc1, InterconnectionAcc2, InterconnectionAcc3, Interconnectionname1, Interconnectionname2, Interconnectionname3 };
    } catch (error) {
        ...
        if (interconnectionAccounts[i]) {                     //interconnectionMeters[i] && 
            ...
            try {
                ...
                const fields = {
                    ...
                    Account: Account,
                    Meter: Meter,
                    Signing_date: dateToday,
                    testingdate: dateToday,
                ...
                fs.writeFileSync('./filed.pdf', pdfBytes);
                delay(3000);
                        ...
                        if (fs.existsSync(pngPath)) {
                           ...
                           const filedPath = path.resolve('/tmp', 'filed.pdf');; // define pdfPath outside the functions
                            ...
                            const pagesData = [
                                ...
                                { pageNum: 11, imageX: 1.7909 * 72, imageY: 3.3126 * 72, imageWidth: 1.6833 * 72, imageHeight: 0.4874 * 72 },
                                { pageNum: 12, imageX: 0.6091 * 72, imageY: 4.9399 * 72, imageWidth: 1.6833 * 72, imageHeight: 0.4874 * 72 },
                                { pageNum: 17, imageX: 5.9182 * 72, imageY: 4.9762 * 72, imageWidth: 1.4742 * 72, imageHeight: 0.3965 * 72 }, // changed position and dimensions
                                { pageNum: 26, imageX: 2.9 * 72, imageY: 6.4581 * 72, imageWidth: 1.6833 * 72, imageHeight: 0.4874 * 72 },
                                { pageNum: 37, imageX: 1.0091 * 72, imageY: 2.2944 * 72, imageWidth: 1.6833 * 72, imageHeight: 0.4874 * 72 },
                            ];
                ...
                let FolderID = 'Interconnection_Initial';
                ...
                for (const [index, array] of arrays.entries()) {
                    ...
                    if (NYC_LI !== 'NYC') {
                        try {
                            await new Promise(resolve => setTimeout(resolve, 15000)); // 5 seconds delay
                            await uploadpvwattsFile(client, filePath1, signedFormsFolderId);
			    ...
			    let solarprocessPUT = {
                "intercinitialfolderlink": drivelink
            };
