...
async function caspiomain(CIDvalue, accessToken) {
    try {
        ...
        const solarProcessUrl = `https://c1abd578.caspio.com/rest/v2/tables/SolarProcess/records?q.select=Contract_Price%2CPanel%2CBuilding_permit_no%2CSystem_Size_Sold%2CBlock%2CLot%2CBin%2CFlood_Zone%2CNYC_LI%2COath_Ecb_Violation%2CFlood_Zone%2CLandmark%2CBorough%2CStories%2CCB&q.where=CustID=${CIDvalue}`;
        ...
        const getElectricPermitUrl = `https://c1abd578.caspio.com/rest/v2/tables/electricalpermit/records?q.select=applicationLink%2C%20ApplicationNumber%2C%20DateOfOpening%2C%20passdate&q.where=CID=${CIDvalue}`;
...
let Lastname = "";
		...
		const DateOfOpening= combinedData.DateOfOpening;
        const ApplicationNumber= combinedData.ApplicationNumber;
...
if (originalAddress) {
    try {
        ...
        if (results) {
            Address = results.formatted_address || originalAddress; // Fallback to originalAddress if no result

            results.address_components.forEach(component => {
                // Check for borough in the address components
                if (component.types.includes('sublocality') || component.types.includes('sublocality_level_1')) {
                    Borough = component.long_name;
                    City = component.long_name; // Set City as the borough name
                }
            });

            // If borough is not found, then check for city
            if (!Borough) {
                results.address_components.forEach(component => {
            ...
            console.log(`Borough: ${Borough}, City: ${City}`);
        }
    } catch (error) {
...
return { 
    Firstname, Lastname, FullName, Address, House, Street, City, lead, Building_permit_no, 
    Zip, DateOfOpening, ApplicationNumber, Landmark, CB, Panel, NYC_LI, Size, Stories, 
    Borough, contractfileid, Contract_Price, Block, Lot, Bin, Flood_Zone, Oath_Ecb_Violation 
}
    } catch (error) {
...
(async () => {
    ...
    for (let i = 0; i < sheetData.length; i++) {
        try {
            ...
            const contractPath = path.resolve('/tmp',`${Lastname}contract.pdf`); // define nycblanksdownloadedpath outside the functions 
            const filePath = `'/tmp/${Lastname} filled.pdf`;
            ...
            let contractfileid = caspioData.contractfileid;
            			let nocontractidfield = '15pvmI7zlCgIDQx7Kls01tHOY4KTzdX13';
			if (contractfileid && contractfileid.trim() !== '') {
                await downloadFile(drive, contractfileid, contractPath);
			} else {
                await downloadFile(drive, nocontractidfield, contractPath); 				
			};
            //console.log("Successfully connected to Caspio!");
            ...
            const fields = {
                ...
                phone: caspioData.Phone && caspioData.Phone.trim() !== '' ? caspioData.Phone : '631-954-2844',
                email: caspioData.email && caspioData.email.trim() !== '' ? caspioData.email : 'teamdesign@patriotenergysolution.com'
            };
